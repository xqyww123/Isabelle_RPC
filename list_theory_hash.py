...
async def main():
    async with Client(args.repl_addr, args.session, timeout=None) as c:
        ...
        fullnames = await c.load_theory([args.theory, "Isabelle_RPC.List_Theory_Hash_App"])
        print(f"Loaded: {fullnames}", file=sys.stderr, flush=True)

        # Run the app
        await c.run_app("Theory_Hash.list")
        await c._write(args.theory)

        # Read result: REPL_Server.output wraps as [payload, null]; errors as [null, error_string]
        entries, err = await c._feed_and_unpack()
        if err is not None:
